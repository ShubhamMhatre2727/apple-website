...
const Slide = ({details, id, handleVideoEnd, handleOnPlay}) => {
  ...
  return (
    <div ref={ref} className='relative min-w-full'>
        <video className={`pointer-events-none h-96 w-full ${(id==3)?"object-cover":""}`} autoPlay={(id==1)?true:false} muted onEnded={()=>handleVideoEnd(id)} onPlay={()=>handleOnPlay(id-1)}>
        ...
        {
            details.map((txt)=><p className='bg-inherit cursor-pointer'>{txt}</p>)
        }
        </div>
    </div>
  )
